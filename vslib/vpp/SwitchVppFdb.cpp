        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    /*
     * NOTE: VRF binding for BVI intentionally disabled.
     *
     * When enabled, the BVI's connected route (10.0.0.0/24) moves to a
     * separate VRF (e.g. table 1001), which isolates overlay from underlay.
     * This is needed for L3 VxLAN failover routing (/32 host routes in the
     * overlay VRF), but it breaks test-server → overlay reachability because
     * VPP's cross-VRF lookup + glean adjacency cannot resolve neighbors.
     *
     * TODO: Re-enable once VPP cross-VRF route leaking is fixed, or when
     * the test topology provides a proper overlay gateway for the test-server.
     */
        ...
        _In_ const sai_attribute_t *attr_list)
{
    ...
    /* Check bridge port type — tunnel bridge ports use TUNNEL_ID, not PORT_ID.
     * For tunnel BPs, resolve the VxLAN tunnel's VPP sw_if_index and program
     * l2fib directly on the tunnel interface. */
    {
        ...
        if (it_bp_type != br_port_attrs.end()) {
            ...
            if (bp_type == SAI_BRIDGE_PORT_TYPE_TUNNEL) {
                /* Resolve tunnel OID → VPP sw_if_index → interface name */
                auto meta_tun = sai_metadata_get_attr_metadata(SAI_OBJECT_TYPE_BRIDGE_PORT,
                    SAI_BRIDGE_PORT_ATTR_TUNNEL_ID);
                auto it_tun = br_port_attrs.find(meta_tun->attridname);
                if (it_tun == br_port_attrs.end()) {
                    SWSS_LOG_ERROR("vpp_fdbentry_add: tunnel bridge port %s missing TUNNEL_ID",
                        sai_serialize_object_id(br_port_id).c_str());
                    return SAI_STATUS_FAILURE;
                }
                sai_object_id_t tunnelOid = it_tun->second->getAttr()->value.oid;
                uint32_t tun_sw_if_index = 0;
                uint16_t tun_vlan_id = 0;
                if (!m_tunnel_mgr.getL2TunnelInfo(tunnelOid, tun_sw_if_index, tun_vlan_id)) {
                    SWSS_LOG_ERROR("vpp_fdbentry_add: tunnel %s not found in TunnelManager",
                        sai_serialize_object_id(tunnelOid).c_str());
                    return SAI_STATUS_FAILURE;
                }
                const char *tun_ifname = vpp_get_swif_name(tun_sw_if_index);
                if (tun_ifname == nullptr) {
                    SWSS_LOG_ERROR("vpp_fdbentry_add: VPP sw_if_index %u has no name", tun_sw_if_index);
                    return SAI_STATUS_FAILURE;
                }
                /* Resolve BD ID from FDB entry's bridge VLAN */
                uint32_t tun_bd_id = tun_vlan_id;
                if (tun_bd_id == 0) {
                    ...
                    if (get(SAI_OBJECT_TYPE_VLAN, bv_id, 1, &vlan_attr) == SAI_STATUS_SUCCESS) {
                        tun_bd_id = vlan_attr.value.u16;
                    }
                }
                auto ret = l2fib_add_del(tun_ifname, fdb_entry.mac_address, tun_bd_id, is_add, is_static);
                SWSS_LOG_NOTICE("vpp_fdbentry_add: tunnel FDB ADD mac=%02x:%02x:%02x:%02x:%02x:%02x "
                    "on %s (sw_if=%u) bd=%u static=%d ret=%d",
        ...
        _In_ const std::string &serializedObjectId)
{
    ...
    /* Check bridge port type — tunnel bridge ports use TUNNEL_ID.
     * For tunnel BPs, resolve VxLAN tunnel sw_if_index and delete from VPP l2fib. */
    {
        auto meta_bp_type = sai_metadata_get_attr_metadata(SAI_OBJECT_TYPE_BRIDGE_PORT, SAI_BRIDGE_PORT_ATTR_TYPE);
        auto it_bp_type = br_port_attrs.find(meta_bp_type->attridname);
        if (it_bp_type != br_port_attrs.end()) {
            ...
            if (bp_type == SAI_BRIDGE_PORT_TYPE_TUNNEL) {
                ...
                auto it_tun = br_port_attrs.find(meta_tun->attridname);
                if (it_tun == br_port_attrs.end()) {
                    SWSS_LOG_ERROR("vpp_fdbentry_del: tunnel bridge port %s missing TUNNEL_ID",
                        sai_serialize_object_id(br_port_id).c_str());
                    return SAI_STATUS_FAILURE;
                }
                sai_object_id_t tunnelOid = it_tun->second->getAttr()->value.oid;
                ...
                uint16_t tun_vlan_id = 0;
                if (!m_tunnel_mgr.getL2TunnelInfo(tunnelOid, tun_sw_if_index, tun_vlan_id)) {
                    SWSS_LOG_ERROR("vpp_fdbentry_del: tunnel %s not found in TunnelManager",
                        sai_serialize_object_id(tunnelOid).c_str());
                    return SAI_STATUS_FAILURE;
                }
                const char *tun_ifname = vpp_get_swif_name(tun_sw_if_index);
                if (tun_ifname == nullptr) {
                    SWSS_LOG_ERROR("vpp_fdbentry_del: VPP sw_if_index %u has no name", tun_sw_if_index);
                    return SAI_STATUS_FAILURE;
                }
                uint32_t tun_bd_id = tun_vlan_id;
                if (tun_bd_id == 0) {
                ...
                auto ret = l2fib_add_del(tun_ifname, fdb_entry.mac_address, tun_bd_id, is_add, is_static);
                SWSS_LOG_NOTICE("vpp_fdbentry_del: tunnel FDB DEL mac=%02x:%02x:%02x:%02x:%02x:%02x "
                    "on %s (sw_if=%u) bd=%u ret=%d",
                    fdb_entry.mac_address[0], fdb_entry.mac_address[1], fdb_entry.mac_address[2],
                    fdb_entry.mac_address[3], fdb_entry.mac_address[4], fdb_entry.mac_address[5],
